...
def setup_chrome_driver():
    """
    设置 Chrome 驱动，如果不存在则自动下载
    
    返回:
        tuple: (chrome_path, driver_path) Chrome 二进制路径和驱动路径
    """
    import os
    import platform
    import zipfile
    ...
    # 创建本地浏览器目录
    browser_dir = os.path.join(os.getcwd(), "local_browser")
    os.makedirs(browser_dir, exist_ok=True)
    
    # 根据操作系统确定 Chrome 和驱动的相关信息
    system = platform.system().lower()
    if system == "windows":
        chrome_dir = os.path.join(browser_dir, "chrome")
        driver_dir = os.path.join(browser_dir, "driver")
        chrome_url = "https://storage.googleapis.com/chrome-for-testing-public/126.0.6478.126/win64/chrome-win64.zip"
        driver_url = "https://storage.googleapis.com/chrome-for-testing-public/126.0.6478.126/win64/chromedriver-win64.zip"
        ...
        if arch == 'arm64':
            chrome_url = "https://storage.googleapis.com/chrome-for-testing-public/126.0.6478.126/mac-arm64/chrome-mac-arm64.zip"
            driver_url = "https://storage.googleapis.com/chrome-for-testing-public/126.0.6478.126/mac-arm64/chromedriver-mac-arm64.zip"
            chrome_path = os.path.join(chrome_dir, "chrome-mac-arm64", "Google Chrome for Testing.app", "Contents", "MacOS", "Google Chrome for Testing")
            driver_path = os.path.join(driver_dir, "chromedriver-mac-arm64", "chromedriver")
        else:
            chrome_url = "https://storage.googleapis.com/chrome-for-testing-public/126.0.6478.126/mac-x64/chrome-mac-x64.zip"
            driver_url = "https://storage.googleapis.com/chrome-for-testing-public/126.0.6478.126/mac-x64/chromedriver-mac-x64.zip"
            chrome_path = os.path.join(chrome_dir, "chrome-mac-x64", "Google Chrome for Testing.app", "Contents", "MacOS", "Google Chrome for Testing")
        ...
        driver_url = "https://storage.googleapis.com/chrome-for-testing-public/120.0.6099.109/linux64/chromedriver-linux64.zip"
    ...
    # 检查 Chrome 是否存在，如果不存在则下载
    if not os.path.exists(chrome_path):
        print("Chrome 不存在，开始下载...")
        download_and_extract_browser(chrome_url, chrome_dir, "chrome")
    
    # 检查下载后的 Chrome 路径是否存在
    if not os.path.exists(chrome_path):
        # 尝试在解压目录中查找 Chrome 可执行文件
        chrome_path = find_chrome_executable(chrome_dir, system)
        if not chrome_path:
            raise FileNotFoundError(f"无法找到 Chrome 可执行文件在 {chrome_dir}")
    
    print(f"使用 Chrome: {chrome_path}")
    
    # 检查 ChromeDriver 是否存在，如果不存在则下载
    if not os.path.exists(driver_path):
        print("ChromeDriver 不存在，开始下载...")
        download_and_extract_browser(driver_url, driver_dir, "driver")
    
    # 检查下载后的 Driver 路径是否存在
    if not os.path.exists(driver_path):
        # 尝试在解压目录中查找 Driver 可执行文件
        driver_path = find_driver_executable(driver_dir, system)
        if not driver_path:
            raise FileNotFoundError(f"无法找到 ChromeDriver 可执行文件在 {driver_dir}")
    ...
    if system != "windows":
        ...
        if os.path.exists(chrome_path):
            st = os.stat(chrome_path)
            os.chmod(chrome_path, st.st_mode | stat.S_IEXEC)
    
    return chrome_path, driver_path


def find_chrome_executable(chrome_dir, system):
    """在解压目录中查找 Chrome 可执行文件"""
    import os
    
    if system == "windows":
        # 在 Windows 上查找 chrome.exe
        for root, dirs, files in os.walk(chrome_dir):
            if "chrome.exe" in files:
                return os.path.join(root, "chrome.exe")
    elif system == "darwin":
        # 在 macOS 上查找 Google Chrome for Testing
        for root, dirs, files in os.walk(chrome_dir):
            if "Google Chrome for Testing" in files and root.endswith("MacOS"):
                return os.path.join(root, "Google Chrome for Testing")
    else:  # linux
        # 在 Linux 上查找 chrome
        for root, dirs, files in os.walk(chrome_dir):
            if "chrome" in files:
...
def find_driver_executable(driver_dir, system):
    """在解压目录中查找 ChromeDriver 可执行文件"""
    import os
    ...
    for root, dirs, files in os.walk(driver_dir):
        if driver_name in files:
            full_path = os.path.join(root, driver_name)
            if system == "windows" or os.access(full_path, os.X_OK):
                return full_path
    
    return None


def download_and_extract_browser(url, extract_dir, component_type):
    """
    下载并解压浏览器组件
    ...
    参数:
        ...
        component_type: str - 组件类型 ('chrome' 或 'driver')
    """
    import os
    import zipfile
    import requests
    ...
    print(f"正在下载 {component_type}... URL: {url}")
    ...
    try:
        ...
        print(f"{component_type} 下载和解压完成")
        
    except Exception as e:
        print(f"\n下载 {component_type} 失败: {e}")
        raise
